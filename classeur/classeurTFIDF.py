...
class ClasseurTFIDF(Classeur):
    """Cette classe attribue comme score aux termes, leur tfidf d'aprés un 
    corpus de référence et selon l'agrégation choisi dans la config.
    ...
    def noter(self,indexCorpus):
        ...
        Le score correspond au tfidf standrad ou log selon le choix fait dans 
        le fichier de config, normaliser par document puis pris selon l'agrégation
        choisi dans la config.
        """
        ...
        for doc,dictTermeFreq in index.items():
            #dict[tuples[str*],float] -> dictionnaire score tfidf du terme dans le doc 
            tmp = dict()
